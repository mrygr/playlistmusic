...
// interface song que representa la estructura de una canción, con propiedades como id, título, artista, portada, duración y URL de la canción.
// interface playlist que representa la estructura de una playlist, con propiedades como id, nombre, descripción, portada y una lista de canciones.
// interface GlobalMenuState que representa el estado del menú contextual global para las canciones, incluyendo la canción seleccionada, la posición del menú y opciones relacionadas.
// interface GlobalModalState que representa el estado del modal global para la creación de playlists, indicando si se muestra o no y la canción inicial opcional para la creación de la playlist.
...
// PlaylistService es un servicio centralizado que maneja la lógica relacionada con las playlists y canciones de la aplicación.
// Proporciona métodos para crear, eliminar y modificar playlists, así como para abrir y cerrar menús globales y modales relacionados con las canciones.
// También mantiene un estado global para el menú contextual de canciones y el modal de creación de playlists, lo que permite una gestión eficiente de la interfaz de usuario en toda la aplicación.
// Contiene una lista pre-cargada de canciones y playlists de ejemplo para facilitar el desarrollo y las pruebas.
...
export class PlaylistService {
  ...
  public allSongs: Song[] = [
    ...
    { id: 12, title: 'Chill Lo-Fi', artist: 'SoundHelix', cover: 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?auto=format&fit=crop&w=300&q=80', duration: '5:33', url: 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-12.mp' }
  ];
  ...
  private playlistsList = signal<Playlist[]>([
    {
      ...
      songs: [this.allSongs[4], this.allSongs[9], this.allSongs[8], this.allSongs[10], this.allSongs[0], this.allSongs[1], this.allSongs[2], this.allSongs[3], this.allSongs[5], this.allSongs[6], this.allSongs[7], this.allSongs[11]] // Deep Ocean, Space Drift, Chill Lo-Fi, Desert Wind, Epic Journey, Cyber Punk
    },
    {
      id: 2,
      name: 'Night Drive',
  ...
  openGlobalSongMenu(song: Song, event: MouseEvent, allowCreate: boolean, excludePlaylistId?: number) {
    ...
    // Lógica para determinar la posición del menú contextual, asegurando que siempre esté completamente visible en la pantalla, incluso cerca de los bordes.
    ...
    if (rect.top + estimatedHeight > window.innerHeight - 15) {
      verticalPos = 'bottom';
      // Anclamos el menú desde ABAJO. Así crece hacia arriba naturalmente.
